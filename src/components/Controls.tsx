...
export function Controls({
  ...
  return (
    <div className="bg-gray-100 border-b border-gray-300 px-3 py-2 flex items-center gap-3 flex-wrap shrink-0">
      ...
      {/* Autoscroll speed */}
      <div className="flex items-center gap-1 bg-white border border-gray-300 rounded-lg px-2 py-1.5">
        <span className="text-xs text-gray-500 px-1">Autoscroll</span>
        <button
          onClick={() => { const v = Math.max(scrollSpeed - 1, 0); setScrollSpeed(v); if (v === 0) setIsScrolling(false); }}
          className="p-1 hover:bg-gray-100 rounded transition"
        ...
        <button
          onClick={() => setScrollSpeed(Math.min(scrollSpeed + 1, 10))}
          className="p-1 hover:bg-gray-100 rounded transition"
      ...
      {/* Start / Pause — only shown when speed > 0 */}
      {scrollSpeed > 0 && (
        <button
